...
class joueur(object):
    ...
    def draw(self, win):
        ...
        if self.estDonneur or self.estPetiteBlinde or self.estGrosseBlinde:
            text_str = ''
            font = pygame.font.SysFont('comic sans MS', 16)

            if self.estDonneur:
                text_str = 'Donneur'
            elif self.estPetiteBlinde:
                text_str = 'P Blinde'
            elif self.estGrosseBlinde:
                text_str = 'G Blinde'
...
class carte(object):
    ...
    def set_joueur_carte(self, bPlayer):
...
class mise(object):
    ...
    def set_mise(self, mise_type, mise_amount):
        self.mise_type = mise_type
        self.mise_amount = mise_amount
    ...
    def draw(self, win):
        if self.IsVisible:
            mise_color = (233, 233, 233)

            if self.mise_type == 'Relance' or self.mise_type == 'Suivre' or self.mise_type == 'All_In':
                mise_str = self.mise_type + ', ' + str(self.mise_amount)
            elif self.mise_type == 'Couche':
                mise_str = self.mise_type
                mise_color = (255, 0, 0)
            elif self.mise_type == 'Check':
                mise_str = self.mise_type
                mise_color = (255, 255, 60)
            else:
                mise_str = "Mauvaise mise"

            font = pygame.font.SysFont('comic sans MS', 22)
            text = font.render(mise_str, True, mise_color)
            win.blit(text, (self.x, self.y))
...
class jeton(object):
    def __init__(self, text, x, y, radius, arc_radius, pos_y, screen_width, screen_height, color, color_border,
                 text_color):
        self.text = text
        self.x = x
        self.y = y
    ...
    def draw(self, win):
        if self.IsVisible:
            if (self.screen_height // 2 > self.pos_y and self.y > self.pos_y) or (
                    self.screen_height // 2 < self.pos_y and self.y < self.pos_y):
                self.vel = 0
            ...
            pygame.draw.circle(win, self.color_border, (self.x, self.y), self.radius + self.circle_width - 3,
                               self.circle_width // 2)
            rect = pygame.draw.circle(win, self.color, (self.x, self.y), self.radius, self.circle_width)
            pygame.draw.circle(win, self.color_border, (self.x, self.y), self.radius - self.circle_width,
                               self.circle_width // 2 - 1)
            ...
            pygame.draw.arc(win, self.color, rect_arc, 2 / 5 * pi + self.del_theta, 3 / 5 * pi + self.del_theta,
                            self.arc_width)
            pygame.draw.arc(win, self.color, rect_arc, 4 / 5 * pi + self.del_theta, 5 / 5 * pi + self.del_theta,
                            self.arc_width)
            pygame.draw.arc(win, self.color, rect_arc, 6 / 5 * pi + self.del_theta, 7 / 5 * pi + self.del_theta,
                            self.arc_width)
            pygame.draw.arc(win, self.color, rect_arc, 8 / 5 * pi + self.del_theta, 9 / 5 * pi + self.del_theta,
                            self.arc_width)
...
class resultat_gagnant(object):
    def __init__(self, x, y):
        self.x = x
        self.y = y
        self.vel = 4
        self.result_text = ''
        self.result_carte_imgs = []
        self.IsVisible = False
    ...
    def set_carte_imgs(self, img_list):
        self.result_carte_imgs = img_list
    ...
    def draw(self, win):
        if self.IsVisible:
            ...
            # draw result cartes
            for idx in range(len(self.result_carte_imgs)):
                win.blit(self.result_carte_imgs[idx], (self.x + 60 * (idx), self.y + 32))


class resultat_gagnant_forfait(object):
    def __init__(self, x, y):
        self.x = x
        self.y = y
        self.vel = 4
